...
func InferOrLoadParquetSchema(query string, srcCfg SourceConfig, sinkCfg map[string]interface{}) (*ParquetSchema, error) {
	...
	_, hasHTTPPort := sinkCfg["port"]
	_, hasDiskSpill := sinkCfg["disk_spill_path"]
	preferHTTPPull := hasHTTPPort || hasDiskSpill

	if preferHTTPPull {
		if hcfg, err := ParseHTTPPullSinkConfig(sinkCfg); err == nil {
	...
	} else if pcfg, err := ParseParquetSinkConfig(sinkCfg); err == nil {
		cachePath = strings.TrimSpace(pcfg.SchemaCachePath)
		if cachePath == "" {
			cachePath = pcfg.Path + ".schema.json"
			if filepath.Dir(cachePath) == string(os.PathSeparator) {
				cachePath = filepath.Join(os.TempDir(), filepath.Base(cachePath))
			}
		}
...
func inferParquetSchemaFromLogicalPlan(node ir.LogicalNode, sourceSchema map[string]string) (*ParquetSchema, error) {
	return inferParquetSchemaWithCTEs(node, sourceSchema, nil)
}

func inferParquetSchemaWithCTEs(node ir.LogicalNode, sourceSchema map[string]string, ctes map[string]ir.LogicalNode) (*ParquetSchema, error) {
	...
	case *ir.LogicalProject:
		if len(n.Columns) == 0 && len(n.Exprs) == 0 {
			return inferParquetSchemaWithCTEs(n.Input, sourceSchema, ctes)
		}
		for _, c := range n.Columns {
			addCol(c, inferTypeFromHint(c))
		}
		for _, e := range n.Exprs {
			colType := inferTypeFromHint(e.As)
	...
	case *ir.LogicalView:
		return inferParquetSchemaWithCTEs(n.Input, sourceSchema, ctes)
	case *ir.LogicalWith:
		return inferParquetSchemaWithCTEs(n.Body, sourceSchema, n.CTEs)
	case *ir.LogicalCTERef:
		...
		return inferParquetSchemaWithCTEs(cte, sourceSchema, ctes)
	case *ir.LogicalScan:
		for col := range sourceSchema {
			addCol(col, inferTypeFromHint(col))
		}
		// Keep deterministic order.
		sort.Slice(cols, func(i, j int) bool { return cols[i].Name < cols[j].Name })
		return &ParquetSchema{Columns: cols}, nil
	default:
		// Fall back to empty schema for unsupported logical nodes.
		return &ParquetSchema{Columns: cols}, nil
	}
