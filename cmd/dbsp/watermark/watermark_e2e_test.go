...
func TestWatermarkE2E_DropLateEvents_DropsOutOfOrderEvent(t *testing.T) {
	...
	wm, err := BuildWatermarkConfig(config.WatermarkYAMLConfig{Enabled: true, Policy: "drop", MaxOutOfOrderness: "0s", AllowedLateness: "0s"})
	if err != nil {
...
func TestWatermarkE2E_EmitLateEvents_MarksLateOutput(t *testing.T) {
	...
	wm, err := BuildWatermarkConfig(config.WatermarkYAMLConfig{Enabled: true, Policy: "emit", MaxOutOfOrderness: "0s", AllowedLateness: "9s"})
	if err != nil {
		t.Fatalf("BuildWatermarkConfig: %v", err)
	}
	ApplyWatermarkConfig(root, wm)
