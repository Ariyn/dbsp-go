...
func TestInferOrLoadParquetSchema_CachesAndReuses(t *testing.T) {
	...
	// Must include group key + agg delta.
...
func TestInferOrLoadParquetSchema_WithCTEBodyUsesCTEColumns(t *testing.T) {
	dir := t.TempDir()
	cache := filepath.Join(dir, "schema.json")
	...
	SELECT id, v2
	FROM cte1`
	src := config.SourceConfig{Type: "http", Config: map[string]interface{}{"schema": map[string]string{"id": "int", "v": "float", "other": "string"}}}
	sinkCfg := map[string]interface{}{"path": filepath.Join(dir, "out"), "schema_cache_path": cache}
	...
	if cols["v2"] != "string" {
		t.Fatalf("expected v2=string, got %q", cols["v2"])
	}
	if _, ok := cols["other"]; ok {
		t.Fatalf("did not expect unrelated source column 'other' in inferred schema: %v", cols)
	}
}
