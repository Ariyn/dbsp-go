...
func TestE2E_Parquet_FinalSnapshotEqualsExpected_GroupByMultiAgg(t *testing.T) {
	...
	src := testutil.NewSliceSource(batches)
	execute := func(b types.Batch) (types.Batch, error) { return op.Execute(root, b) }
	if err := pipeline.RunPipeline(context.Background(), src, pqSink, execute, nil, nil, 0); err != nil {
		t.Fatalf("pipeline.RunPipeline: %v", err)
	}
	if err := pqSink.Close(); err != nil {
