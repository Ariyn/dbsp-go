...
func BenchmarkBaseline_GroupAgg(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		source, err := NewCSVSource(sourceCfg)
		if err != nil {
			b.Fatalf("NewCSVSource: %v", err)
		}
...
func BenchmarkBaseline_JoinGroupAgg(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		sourceA, _ := NewCSVSource(sourceACfg)
		sourceB, _ := NewCSVSource(sourceBCfg)
...
func BenchmarkBaseline_E2E_Parquet(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		sink, _ := NewParquetSink(sinkCfg, schema)

		source, _ := NewCSVSource(sourceCfg)
