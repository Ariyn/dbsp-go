...
func TestRunPipeline_SQL_MultiAgg_DeleteRetraction(t *testing.T) {
	...
	// Source -> pipeline.RunPipeline -> SQL compile -> op.Execute -> Sink
	...
	snk := testutil.NewRecordingSink()

	execute := func(b types.Batch) (types.Batch, error) {
		return op.Execute(root, b)
	}

	if err := RunPipeline(context.Background(), src, snk, execute, nil, nil, 0); err != nil {
		t.Fatalf("RunPipeline: %v", err)
	}

	if got, want := len(snk.Batches), 2; got != want {
		t.Fatalf("expected %d sink batches, got %d", want, got)
	}

	assertSingleTupleDelta(t, snk.Batches[0], types.Tuple{"k": "A", "agg_delta": 10.0, "count_delta": int64(1)})
	assertSingleTupleDelta(t, snk.Batches[1], types.Tuple{"k": "A", "agg_delta": -10.0, "count_delta": int64(-1)})
}
