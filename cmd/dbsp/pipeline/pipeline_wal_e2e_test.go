...
func snapshotFromSink(s *testutil.RecordingSink) map[string]aggSnapshot {
...
func TestE2E_WALCheckpointRestore_FinalSnapshotMatchesBaseline_GroupAgg(t *testing.T) {
	...
	if err := RunPipeline(context.Background(), testutil.NewSliceSource(batches), baseSink, func(b types.Batch) (types.Batch, error) {
	...
	firstSink := testutil.NewRecordingSink()

	checkpointEvery := 3
	if err := RunPipeline(context.Background(), testutil.NewSliceSource(batches[:4]), firstSink, func(b types.Batch) (types.Batch, error) {
	...
	secondSink := testutil.NewRecordingSink()

	if err := RunPipeline(context.Background(), testutil.NewSliceSource(batches[4:]), secondSink, func(b types.Batch) (types.Batch, error) {
...
func applyAggDeltasFromSink(s *testutil.RecordingSink, snap map[string]aggSnapshot) {
	for _, b := range s.Batches {
